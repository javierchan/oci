"""Justifications router — deterministic narratives and template governance."""
...
async def list_prompt_templates(db: AsyncSession = Depends(get_db)) -> PromptTemplateVersionListResponse:
    """Return governed prompt/narrative template versions."""
...
@router.post(
    ...
    summary="Create narrative template",
)
async def create_prompt_template(
    ...
    """Create a governed deterministic narrative template version."""
...
@router.get(
    "/templates/{version}",
    response_model=PromptTemplateVersionResponse,
    summary="Get narrative template",
)
async def get_prompt_template(
    version: str,
    db: AsyncSession = Depends(get_db),
) -> PromptTemplateVersionResponse:
    """Return one governed deterministic narrative template."""
...
@router.patch(
    "/templates/{version}",
    response_model=PromptTemplateVersionResponse,
    summary="Update narrative template",
)
async def update_prompt_template(
    ...
    """Patch one governed deterministic narrative template version."""
...
@router.post(
    ...
    response_model=PromptTemplateVersionResponse,
    summary="Set default narrative template",
)
...
async def list_justifications(
    ...
    """List deterministic justification narratives for one project."""
...
async def get_justification(
    ...
    """Return the governed or draft deterministic narrative for one integration."""
...
async def approve_justification(
    project_id: str,
    integration_id: str,
    body: ApproveJustificationRequest | None = None,
    db: AsyncSession = Depends(get_db),
    actor_id: str = Header("api-user", alias="X-Actor-Id"),
) -> JustificationRecordResponse:
    """Persist the deterministic narrative and mark it approved."""

    request = body or ApproveJustificationRequest()
    request_actor_id = request.actor_id if body is not None and request.actor_id != "api-user" else actor_id
    async with db.begin():
...
async def override_justification(
    ...
    """Persist an architect override while retaining deterministic evidence."""

    request_actor_id = body.actor_id if body.actor_id != "api-user" else actor_id
    async with db.begin():
...
@router.delete(
    "/{project_id}/{integration_id}",
    response_model=JustificationRecordResponse,
    summary="Reset justification narrative",
)
async def reset_justification(
    project_id: str,
    integration_id: str,
    body: ResetJustificationRequest | None = None,
    db: AsyncSession = Depends(get_db),
    actor_id: str = Header("api-user", alias="X-Actor-Id"),
) -> JustificationRecordResponse:
    """Delete a governed record and return the regenerated deterministic draft."""

    request = body or ResetJustificationRequest()
    request_actor_id = request.actor_id if body is not None and request.actor_id != "api-user" else actor_id
    async with db.begin():
