...
def test_specialized_agents_share_explainable_output_and_safety_contracts() -> None:
    ...
    assert get_agent_definition("service_verification").version == "1.1.0"
    assert "when no source was retrieved" in get_agent_definition("service_verification").instruction
    assert get_agent_definition("import_quality").version == "1.1.0"
    assert get_agent_definition("topology_investigation").version == "1.1.0"
    assert get_agent_definition("bom_scenario").version == "1.1.0"
