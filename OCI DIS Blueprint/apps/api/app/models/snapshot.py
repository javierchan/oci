...
class VolumetrySnapshot(Base, UUIDMixin, TimestampMixin):
    """Immutable result of a full volumetry recalculation for a project."""
    ...
    row_results: Mapped[dict] = mapped_column(JSON, nullable=False)
    consolidated: Mapped[dict] = mapped_column(JSON, nullable=False)
    snapshot_metadata: Mapped[Optional[dict]] = mapped_column("metadata", JSON)


class DashboardSnapshot(Base, UUIDMixin, TimestampMixin):
    """Immutable dashboard render — sourced from VolumetrySnapshot."""

    __tablename__ = "dashboard_snapshots"

    project_id: Mapped[str] = mapped_column(ForeignKey("projects.id"), nullable=False)
    volumetry_snapshot_id: Mapped[str] = mapped_column(ForeignKey("volumetry_snapshots.id"), nullable=False)
    mode: Mapped[str] = mapped_column(String(50), default="technical")
...
class JustificationRecord(Base, UUIDMixin, TimestampMixin):
    """Deterministic narrative record per integration."""
    ...
    deterministic_text: Mapped[dict] = mapped_column(JSON, nullable=False)
...
class AuditEvent(Base, UUIDMixin, TimestampMixin):
    """Structured audit trail — every write emits an event."""
