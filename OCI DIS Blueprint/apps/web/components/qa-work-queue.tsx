...
export function QaWorkQueue({ projectId, reviewCount, pendingCount, risks }: QaWorkQueueProps): JSX.Element | null {
  ...
  const href = `/projects/${projectId}/catalog?qa_status=${pendingCount > 0 ? "PENDING" : "REVISAR"}`;
  return (
    <section aria-labelledby="qa-work-queue-title" className="app-card border-[var(--color-qa-revisar-border)] p-5">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div className="flex min-w-0 gap-3">
          <span className="mt-0.5 rounded-lg bg-[var(--color-qa-revisar-bg)] p-2 text-[var(--color-qa-revisar-text)]">
            <CircleAlert className="h-5 w-5" />
          </span>
          <div>
            <p className="app-label text-[var(--color-qa-revisar-text)]">QA work queue</p>
