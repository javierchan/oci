    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-5 xl:grid-cols-3">
        {sections.map((section) => (
          <section key={section.title} className="rounded-[1.5rem] border border-slate-200 bg-white p-5">
            <p className="text-xs uppercase tracking-[0.25em] text-slate-500">{section.title}</p>
            <dl className="mt-4 space-y-3">
              {section.rows.map(([label, value]) => (
                <div key={label} className="rounded-2xl border border-slate-100 bg-slate-50 px-4 py-3">
                  ...
                  <dd className="mt-2 text-sm font-medium text-slate-900">{stringifyValue(value)}</dd>
                </div>
              ))}
            </dl>
          </section>
      ...
      {duplicates.length > 0 ? (
        <section className="rounded-[1.5rem] border border-amber-300 bg-amber-50 p-5">
          <p className="text-xs uppercase tracking-[0.25em] text-amber-700">Duplicate Warning</p>
