...
function firstEnvironment(draft: DeploymentScenarioCreate): DeploymentEnvironmentInput {
  return draft.environments[0] ?? {
    name: "Production",
...
export function BomWorkspace({ projectId, projectName }: { projectId: string; projectName: string }): JSX.Element {
  ...
  const environment = draft ? firstEnvironment(draft) : null;
  const blockedCount = selectedSnapshot?.line_items.filter((line) => line.status === "blocked").length ?? 0;

  function patchDraft(patch: Partial<DeploymentScenarioCreate>): void {
    setDraft((current) => current ? { ...current, ...patch } : current);
  }

  function patchEnvironment(patch: Partial<DeploymentEnvironmentInput>): void {
    setDraft((current) => {
      if (!current) {
        return current;
      }
      return { ...current, environments: [{ ...firstEnvironment(current), ...patch }] };
    });
  }
    ...
    <div className="space-y-5">
      ...
      <section className="grid gap-4 xl:grid-cols-[minmax(0,1.25fr)_minmax(22rem,0.75fr)]">
        <div className="app-card p-5">
          ...
          {draft && environment ? <div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            ...
            <label className="text-sm font-semibold text-[var(--color-text-primary)]">Contract months<input type="number" min={1} max={120} className="mt-2 w-full rounded-lg border border-[var(--color-border)] bg-[var(--color-surface)] px-3 py-2.5" value={draft.contract_months} onChange={(event) => patchDraft({ contract_months: Number(event.target.value) })} /></label>
            <label className="text-sm font-semibold text-[var(--color-text-primary)]">Environment<input className="mt-2 w-full rounded-lg border border-[var(--color-border)] bg-[var(--color-surface)] px-3 py-2.5" value={environment.name} onChange={(event) => patchEnvironment({ name: event.target.value })} /></label>
            <label className="text-sm font-semibold text-[var(--color-text-primary)]">Active hours / month<input type="number" min={0} max={744} className="mt-2 w-full rounded-lg border border-[var(--color-border)] bg-[var(--color-surface)] px-3 py-2.5" value={environment.active_hours_month} onChange={(event) => patchEnvironment({ active_hours_month: Number(event.target.value) })} /></label>
            <label className="text-sm font-semibold text-[var(--color-text-primary)]">Demand share<input type="number" min={0} max={1} step={0.05} className="mt-2 w-full rounded-lg border border-[var(--color-border)] bg-[var(--color-surface)] px-3 py-2.5" value={environment.demand_share} onChange={(event) => patchEnvironment({ demand_share: Number(event.target.value) })} /></label>
            <label className="text-sm font-semibold text-[var(--color-text-primary)]">HA multiplier<input type="number" min={1} max={10} step={0.1} className="mt-2 w-full rounded-lg border border-[var(--color-border)] bg-[var(--color-surface)] px-3 py-2.5" value={environment.ha_multiplier} onChange={(event) => patchEnvironment({ ha_multiplier: Number(event.target.value) })} /></label>
            <label className="text-sm font-semibold text-[var(--color-text-primary)]">DR role<select className="mt-2 w-full rounded-lg border border-[var(--color-border)] bg-[var(--color-surface)] px-3 py-2.5" value={environment.dr_role} onChange={(event) => patchEnvironment({ dr_role: event.target.value as DeploymentEnvironmentInput["dr_role"] })}><option value="primary">Primary</option><option value="standby">Standby</option><option value="none">None</option></select></label>
          </div> : null}

          <div className="mt-5 flex flex-wrap gap-2"><button className="app-button-primary gap-2" type="button" disabled={!draft || busyAction !== null} onClick={() => void createScenario()}>{busyAction === "create-scenario" ? <Loader2 className="h-4 w-4 animate-spin" /> : <Send className="h-4 w-4" />}Save new scenario</button><button className="app-button-secondary gap-2" type="button" disabled={!selectedScenario || selectedScenario.status === "approved" || busyAction !== null} onClick={() => void approveScenario()}>{busyAction === "approve-scenario" ? <Loader2 className="h-4 w-4 animate-spin" /> : <ShieldCheck className="h-4 w-4" />}Approve selected</button></div>
        </div>

        <aside className="app-card p-5">
          <p className="app-label">Assistant Brief</p>
          <div className="mt-3 flex items-center justify-between gap-3"><span className="text-sm font-semibold text-[var(--color-text-primary)]">Confidence</span><span className="rounded-full border border-[var(--color-border)] px-2.5 py-1 text-xs font-semibold text-[var(--color-text-secondary)]">{assistant?.confidence ?? "unavailable"}</span></div>
          <p className="mt-3 text-sm leading-6 text-[var(--color-text-secondary)]">{assistant?.ai_summary ?? "Deterministic evidence is available; AI synthesis is optional and does not change governed values."}</p>
          <p className="app-label mt-5">Detected services</p><div className="mt-2 flex flex-wrap gap-2">{assistant?.detected_services.map((service) => <span key={service} className="rounded-full border border-[var(--color-border)] px-2.5 py-1 text-xs font-semibold text-[var(--color-text-secondary)]">{service}</span>)}</div>
          <p className="app-label mt-5">Client questions</p><ol className="mt-2 space-y-2">{assistant?.required_questions.map((question, index) => <li key={question} className="flex gap-2 text-sm leading-5 text-[var(--color-text-secondary)]"><span className="font-mono text-[var(--color-accent)]">{index + 1}.</span>{question}</li>)}</ol>
          {assistant?.warnings.length ? <div className="mt-4 rounded-lg border border-amber-400/45 p-3 text-sm text-amber-700 dark:text-amber-300"><AlertTriangle className="mr-2 inline h-4 w-4" />{assistant.warnings.join(" ")}</div> : null}
        </aside>
      </section>
      ...
      {selectedSnapshot ? <>
        <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
          {[{ label: "Monthly", value: currency(selectedSnapshot.monthly_total, selectedSnapshot.currency) }, { label: "Annual", value: currency(selectedSnapshot.annual_total, selectedSnapshot.currency) }, { label: "Contract", value: currency(selectedSnapshot.contract_total, selectedSnapshot.currency) }, { label: "Coverage", value: `${selectedSnapshot.coverage_pct.toFixed(1)}%` }, { label: "Blocked lines", value: String(blockedCount) }].map((metric) => <div key={metric.label} className="app-card p-4"><p className="app-label">{metric.label}</p><p className="mt-2 text-2xl font-semibold text-[var(--color-text-primary)]">{metric.value}</p></div>)}
        </section>
      ...
      {snapshots.length > 1 ? <section className="app-card p-5"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="app-label">Snapshot Comparison</p><h2 className="mt-2 text-xl font-semibold text-[var(--color-text-primary)]">Explain estimate movement</h2></div><button type="button" className="app-button-secondary gap-2" disabled={busyAction !== null || baselineSnapshotId === comparisonSnapshotId} onClick={() => void compareSnapshots()}>{busyAction === "compare" ? <Loader2 className="h-4 w-4 animate-spin" /> : <RefreshCcw className="h-4 w-4" />}Compare</button></div><div className="mt-4 grid gap-3 md:grid-cols-2"><label className="text-sm font-semibold text-[var(--color-text-primary)]">Baseline<select className="mt-2 w-full rounded-lg border border-[var(--color-border)] bg-[var(--color-surface)] px-3 py-2.5" value={baselineSnapshotId} onChange={(event) => { setBaselineSnapshotId(event.target.value); setComparison(null); }}>{snapshots.map((snapshot) => <option key={snapshot.id} value={snapshot.id}>{formatDate(snapshot.created_at)} · {currency(snapshot.monthly_total, snapshot.currency)}</option>)}</select></label><label className="text-sm font-semibold text-[var(--color-text-primary)]">Comparison<select className="mt-2 w-full rounded-lg border border-[var(--color-border)] bg-[var(--color-surface)] px-3 py-2.5" value={comparisonSnapshotId} onChange={(event) => { setComparisonSnapshotId(event.target.value); setComparison(null); }}>{snapshots.map((snapshot) => <option key={snapshot.id} value={snapshot.id}>{formatDate(snapshot.created_at)} · {currency(snapshot.monthly_total, snapshot.currency)}</option>)}</select></label></div>{comparison ? <div className="mt-5 border-t border-[var(--color-border)] pt-5"><div className="grid gap-3 sm:grid-cols-3"><div><p className="app-label">Monthly delta</p><p className="mt-2 text-xl font-semibold text-[var(--color-text-primary)]">{currency(comparison.monthly_delta, comparison.currency)}</p></div><div><p className="app-label">Annual delta</p><p className="mt-2 text-xl font-semibold text-[var(--color-text-primary)]">{currency(comparison.annual_delta, comparison.currency)}</p></div><div><p className="app-label">Contract delta</p><p className="mt-2 text-xl font-semibold text-[var(--color-text-primary)]">{currency(comparison.contract_delta, comparison.currency)}</p></div></div><div className="mt-5 grid gap-5 lg:grid-cols-3"><div><p className="app-label">Service deltas</p><div className="mt-2 space-y-2">{Object.entries(comparison.service_monthly_deltas).map(([label, value]) => <div key={label} className="flex justify-between gap-3 text-sm"><span className="text-[var(--color-text-secondary)]">{label}</span><span className="font-mono text-[var(--color-text-primary)]">{currency(value, comparison.currency)}</span></div>)}</div></div><div><p className="app-label">Environment deltas</p><div className="mt-2 space-y-2">{Object.entries(comparison.environment_monthly_deltas).map(([label, value]) => <div key={label} className="flex justify-between gap-3 text-sm"><span className="text-[var(--color-text-secondary)]">{label}</span><span className="font-mono text-[var(--color-text-primary)]">{currency(value, comparison.currency)}</span></div>)}</div></div><div><p className="app-label">Drivers</p><ul className="mt-2 space-y-2 text-sm leading-5 text-[var(--color-text-secondary)]">{comparison.drivers.map((driver) => <li key={driver}>{driver}</li>)}</ul></div></div></div> : null}<div className="mt-5 border-t border-[var(--color-border)] pt-4"><p className="app-label">Snapshot History</p><div className="mt-3 flex flex-wrap gap-2">{snapshots.map((snapshot) => <button key={snapshot.id} type="button" className={`rounded-lg border px-3 py-2 text-left text-sm ${selectedSnapshot?.id === snapshot.id ? "border-[var(--color-accent)] text-[var(--color-text-primary)]" : "border-[var(--color-border)] text-[var(--color-text-secondary)]"}`} onClick={() => void selectSnapshot(snapshot)}>{formatDate(snapshot.created_at)} · {currency(snapshot.monthly_total, snapshot.currency)}</button>)}</div></div></section> : null}
    </div>
  );
}
