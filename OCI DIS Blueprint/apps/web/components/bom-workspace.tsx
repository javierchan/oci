  ...
  return (
    <div className="space-y-5">
      ...
      {selectedSnapshot ? <>
        ...
        <section className="app-table-shell"><div className="border-b border-[var(--color-border)] px-5 py-4"><p className="app-label">Line Items</p><h2 className="mt-2 text-xl font-semibold text-[var(--color-text-primary)]">Demand, commercial variant, price and provenance</h2></div><div className="overflow-x-auto"><table className="w-full min-w-[1200px] text-left text-sm"><thead className="bg-[var(--color-surface-2)] text-xs uppercase tracking-[0.12em] text-[var(--color-text-muted)]"><tr><th className="px-5 py-3">Environment / service</th><th className="px-5 py-3">Edition / license / SKU</th><th className="px-5 py-3">Metric</th><th className="px-5 py-3 text-right">Quantity</th><th className="px-5 py-3 text-right">Unit price</th><th className="px-5 py-3 text-right">Monthly</th><th className="px-5 py-3">Status / provenance</th></tr></thead><tbody className="divide-y divide-[var(--color-border)]">{selectedSnapshot.line_items.map((line) => { const requiresRateCard = line.status === "rate_card_required"; return <tr key={line.id} className="align-top"><td className="px-5 py-4"><p className="font-semibold text-[var(--color-text-primary)]">{line.description}</p><p className="mt-1 text-xs text-[var(--color-text-muted)]">{line.environment} · {line.service_id}</p></td><td className="px-5 py-4"><p className="font-medium text-[var(--color-text-primary)]">{typeof line.provenance.commercial_variant === "string" ? line.provenance.commercial_variant : "Governed default"}</p><p className="mt-1 font-mono text-xs text-[var(--color-text-muted)]">{line.part_number ?? "Non-billable"}</p></td><td className="px-5 py-4 text-[var(--color-text-secondary)]">{line.metric_name}</td><td className="px-5 py-4 text-right font-mono text-[var(--color-text-primary)]">{formatNumber(line.quantity)} {line.unit}</td><td className="px-5 py-4 text-right font-mono text-[var(--color-text-secondary)]">{requiresRateCard ? "—" : line.unit_price.toFixed(6)}</td><td className="px-5 py-4 text-right font-semibold text-[var(--color-text-primary)]">{requiresRateCard ? "—" : currency(line.monthly_amount, selectedSnapshot.currency)}</td><td className="max-w-sm px-5 py-4"><span className={`rounded-full border px-2.5 py-1 text-xs font-semibold ${statusTone(line.status)}`}>{requiresRateCard ? "Rate card required" : line.status.replaceAll("_", " ")}</span><p className="mt-2 break-words text-xs leading-5 text-[var(--color-text-muted)]">{requiresRateCard ? "Exact customer-rate part-number match required" : line.formula}</p>{line.warnings.length ? <p className="mt-1 text-xs text-amber-700 dark:text-amber-300">{line.warnings.map(warningText).join(" · ")}</p> : null}</td></tr>; })}</tbody></table></div></section>
