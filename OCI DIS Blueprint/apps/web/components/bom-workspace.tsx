  ...
  return (
    <div className="space-y-5">
      ...
      <section className="app-card p-5">
        ...
        {selectedScenario ? <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-6"><div><p className="app-label">Status</p><p className="mt-1 font-semibold text-[var(--color-text-primary)]">{selectedScenario.status}</p></div><div><p className="app-label">Price source</p><p className="mt-1 text-sm text-[var(--color-text-secondary)]">{selectedScenario.price_mode.replace(/_/g, " ")}</p></div><div><p className="app-label">Commercial model</p><p className="mt-1 text-sm text-[var(--color-text-secondary)]">{selectedScenario.commitment_model.replace(/_/g, " ")}</p></div><div><p className="app-label">Region</p><p className="mt-1 text-sm text-[var(--color-text-secondary)]">{selectedScenario.region}</p></div><div><p className="app-label">Currency</p><p className="mt-1 font-mono text-sm text-[var(--color-text-secondary)]">{selectedScenario.currency}</p></div><div><p className="app-label">Updated</p><p className="mt-1 text-sm text-[var(--color-text-secondary)]">{formatDate(selectedScenario.updated_at)}</p></div></div> : <p className="mt-4 text-sm text-[var(--color-text-secondary)]">Create and approve a scenario before generating a BOM.</p>}
      </section>
