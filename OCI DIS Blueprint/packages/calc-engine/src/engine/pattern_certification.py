...
PATTERN_CERTIFICATIONS: dict[str, PatternCertificationProfile] = {
    ...
    "#10": _profile("#10", "CQRS + Event Sourcing", "event_projection", evidence=("idempotency", "retention_processing_window", "data_security_classification"), tools=(("streaming", "functions"), ("streaming", "oic", "data integration")), services=("STREAMING", "FUNCTIONS", "OIC3", "DATA_INTEGRATION", "OBJECT_STORAGE"), controls=("event retention", "projection rebuild", "schema compatibility"), summary="Commands, events, and projections are governed through broker, orchestration or compute, projection processing, and archive demand."),
    ...
    "#16": _profile("#16", "Integration Mesh", "service_mesh", evidence=("business_criticality", "data_security_classification", "target_latency_sla"), tools=(("oic",), ("functions",)), overlays=(("kubernetes engine", "observability", "iam"), ("service mesh", "observability", "iam")), services=("API_GATEWAY", "IAM", "OBSERVABILITY"), external=("OKE and Istio capacity",), controls=("mTLS", "traffic policy", "platform ownership"), summary="The App certifies integration traffic and policy evidence while declaring OKE/Istio as an external capacity dependency."),
