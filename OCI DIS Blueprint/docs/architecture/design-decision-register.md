...
| DD-09 | Accepted | Separate deterministic and agent Celery consumers. | Model latency and provider failures must not starve import, recalculation, or pricing jobs. | `agent-worker` consumes the `agents` queue; late ACK, worker-loss rejection, visibility timeout, result expiry, and low prefetch are application defaults, while managed-broker failover remains M77 evidence. | [ADR-004](../adr/ADR-004-deployment-neutral-multi-user-consistency.md) |
...
| DD-13 | Accepted | Let the App Knowledge Governance Agent regenerate and validate embeddings automatically. | The knowledge derives from executable App contracts and curated App guidance; routine human approval adds delay without a new source of truth. | Publication to the shared S3-compatible active object occurs only after model, dimensions, hash, coverage, and drift checks pass; transactional release history remains M77 work. | [ADR-004](../adr/ADR-004-deployment-neutral-multi-user-consistency.md) |
...
| DD-23 | Accepted | Enforce centralized mutation authorization, reviewed-version concurrency, shared application state, read-only readiness, and deployment-neutral trace correlation before any OCI work. | Multi-user correctness is an application invariant, not a property supplied automatically by Kubernetes or IAM. | Unsafe routes fail closed without policy; stale writes return 409; scheduled jobs use leases; OCI provisioning remains explicitly outside this decision. | [ADR-004](../adr/ADR-004-deployment-neutral-multi-user-consistency.md) |
