...
- Consolidated diagrams honor `--diagram-depth` (1=global regions only, 2=regional abstraction with aggregated network-attached workloads, 3=full workloads + edges). Consolidated flowcharts at depth > 1 render a summary hierarchy with category counts and no per-resource edges.
