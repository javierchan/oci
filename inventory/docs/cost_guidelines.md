...
## FinOps Assessment Alignment (Best-Effort)
...
- Narrative sections SHOULD be generated via OCI GenAI using structured context (time range, currency, totals, counts, and top contributors).
- If GenAI is unavailable or fails, insert a deterministic fallback narrative and record the failure.
...
- Use stable rounding; if Execution Metadata is available in the cost report, document the rounding there.
